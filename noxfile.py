...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/github_runner/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/github_runner/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/github_runner/*")
            ...
            #    "--include=src/saltext/github_runner/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/github_runner/config/schemas",
    )
